...
1. Фото и видео могут прийти из Mini App или из Telegram-бота.
2. Backend принимает `multipart/form-data` из Mini App, а для Telegram скачивает бинарный файл по `file_id`.
3. Фото, обычные видео и `video_note` сходятся в одну папку Google Drive.
4. В таблицу `Фото` пишутся timestamp, user_id, username, имя, источник и метаданные файла в Drive.

### 5.6 Рассадка

1. Клиент может запросить `/api/seating/info` для общей опубликованной рассадки и `/api/seating/personal` для своего стола.
2. Backend читает опубликованную копию рассадки из листа `Рассадка_фикс`.
3. Общий режим возвращает JSON `{ visible, published_at, tables }`, где каждый стол сериализуется как `{ table, guests }`.
4. Персональный режим находит стол и соседей гостя по auth-идентичности и устойчивому сравнению полного имени без зависимости от порядка слов.
5. Кнопка `Обновить рассадку` в админ-панели перезаписывает `Рассадка_фикс` из текущего листа `Рассадка`.
...
- `Фото` — журнал загруженных фото и metadata-связка с Google Drive.
