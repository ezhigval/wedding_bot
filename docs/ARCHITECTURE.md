...
3. Рабочий лист `Рассадка` поддерживает двустороннюю синхронизацию с `Список гостей`: ручные перемещения обновляют стол гостя в колонке `G`, а затем лист канонизируется обратно по алфавиту и статусу подтверждения.
4. В рабочем листе `Рассадка` визуальное оформление ячейки вычисляется из колонок `D/E` листа `Список гостей`.
5. Для снижения расхода Google Sheets quota backend читает `Список гостей` и `Рассадка` пакетно, а при серии правок в `Рассадка` повторно использует краткоживущий кэш строк гостей.
