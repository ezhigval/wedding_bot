...
3. `google_sheets.AddGuestToSheets` ищет существующую строку по имени или Telegram-идентификатору.
4. Подтверждение участия пишется в `Список гостей`.
5. Кэш регистрации обновляется.

### 5.3 Проверка регистрации

1. Frontend вызывает `/api/check-registration`.
2. Backend разрешает identity.
3. Проверка идёт по колонке идентификатора в `Список гостей`.
...
2. Backend определяет полное имя гостя по `user_id`.
3. Данные ищутся в фиксированной схеме `Рассадка_фикс`.
4. Возвращаются стол и соседи.
